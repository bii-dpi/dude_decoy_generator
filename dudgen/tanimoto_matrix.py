...
def dostuff(qf, tf, outf=None, 
    ...
    outf.write("\ntanimoto matrix:\n")
    outf.write(str(tm)+"\n")
    #maxes = [max(tm[i][j] for j in xrange(len(tm[0])) if i != j) for i in xrange(len(tm))]
    maxes = [max(tm[i][j] for j in xrange(len(tm[0]))) for i in xrange(len(tm))]
    outf.write("\nmaximum tanimoto for queries in targets:\n")
    outf.write(str(maxes)+"\n")
    return 0
